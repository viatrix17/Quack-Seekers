...
void drawCube(glm::mat4 wall, std::tuple<float, float, float> scale, std::tuple<float, float, float> color) {
	wall = glm::scale(wall, glm::vec3(std::get<0>(scale),std::get<1>(scale),std::get<2>(scale)));
...
void drawRoom(glm::vec3 cameraOffset, glm::vec3 viewOffset, float roomAngle) {
	...
	room = glm::translate(room, -cameraOffset);
