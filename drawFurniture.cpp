...
void drawWardrobe(glm::mat4 room, std::tuple<float, float, float> color) {
	...
	drawCubeWood(back, std::tuple<float, float, float> (68.0f, 100.0f, 1.0f), color);
	...
	for (float i = -1; i <= 1; i += 2) {
		...
		drawCubeWood(board, std::tuple<float, float, float>(70.0f, 25.0f, 1.0f), color);
	}
	...
	for (float i = -1; i <= 1; i += 2) {
		...
		drawCubeWood(side, std::tuple<float, float, float>(25.0f, 100.0f, 1.0f), color);
	}
	...
	for (float i = -1; i <= 1; i += 2) {
		...
		doorWing = glm::translate(doorWing, glm::vec3(-34.5f*i, 0.0f, 0.0f));
		drawCubeWood(doorWing, std::tuple<float, float, float>(34.0f, 100.0f, 1.0f), color);
	}
	...
	for (float i = 1; i < 5; i++) {
		...
		drawCubeWood(shelf, std::tuple<float, float, float>(70.0f, 25.0f, 1.0f), color);
	}
}
...
void drawBoxes(glm::mat4 desk) {
	...
	for (int i = 0; i < 2; i++) {
		...
		for (int i = -1; i <= 1; i+=2) {
			...
			drawCubeWood(lowerVer, std::tuple<float, float, float>(boxSize.x, boxSize.y, 0.25f), std::tuple<float, float, float>(0.5f, 0.12f, 0.8f));
			...
			drawCubeWood(lowerSide, std::tuple<float, float, float>(boxSize.z-0.25f, boxSize.y, 0.25f), std::tuple<float, float, float>(0.5f, 0.12f, 0.8f));

			glm::mat4 upperVer = upperPart;
			upperVer = glm::translate(upperVer, glm::vec3(0.0f, 0.0f, i * boxSize.z));
			drawCubeWood(upperVer, std::tuple<float, float, float>(boxSize.x, 2.0f, 0.25f), std::tuple<float, float, float>(0.5f, 0.12f, 0.8f));
			...
			drawCubeWood(upperSide, std::tuple<float, float, float>(boxSize.z - 0.25f, 2.0f, 0.25f), std::tuple<float, float, float>(0.5f, 0.12f, 0.8f));
		...
		drawCubeWood(bottom, std::tuple<float, float, float>(boxSize.x, boxSize.z+0.25, 0.25f), std::tuple<float, float, float>(0.5f, 0.12f, 0.8f));

		glm::mat4 top = upperPart;
		top = glm::translate(top, glm::vec3(0.0f, 2.0f, 0.0f));
		top = glm::rotate(top, 90 * PI / 180, glm::vec3(1.0f, 0.0f, 0.0f));
		drawCubeWood(top, std::tuple<float, float, float>(boxSize.x, boxSize.z+0.25, 0.25f), std::tuple<float, float, float>(0.5f, 0.12f, 0.8f));
	}
}
...
void drawDesk(glm::mat4 room, std::tuple<float, float, float> color) {
	...
	drawCubeWood(board, std::tuple<float, float, float>(72.0f, 27.0f, 1.0f), color);
	...
	for (float i = -1; i <= 1; i += 2) {
		...
		drawCubeWood(deskSide, std::tuple<float, float, float>(25.0f, 37.5f, 1.0f), color);
	}
	...
	for (float i = -1; i <= 1; i += 2) {
		...
		drawCubeWood(drawerSide, std::tuple<float, float, float>(20.0f, 10.0f, 1.0f), color);
	}
	
	glm::mat4 drawerFront = drawer;
	drawerFront = glm::translate(drawerFront, glm::vec3(0.0f, 0.0f, -22.0f));
	drawCubeWood(drawerFront, std::tuple<float, float, float>(21.0f, 12.0f, 1.0f), color);

	glm::mat4 drawerBack = drawer;
	drawerBack = glm::translate(drawerBack, glm::vec3(0.0f, 0.0f, 20.0f));
	drawCubeWood(drawerBack, std::tuple<float, float, float>(18.0f, 10.0f, 1.0f), color);
	...
	drawCubeWood(drawerBottom, std::tuple<float, float, float>(18.0f, 21.0f, 1.0f), color);
	...
	drawCubeWood(cabinetSide, std::tuple<float, float, float>(21.0f, 35.0f, 1.0f), color);
	...
	drawCubeWood(cabinetFront, std::tuple<float, float, float>(21.0f, 23.0f, 1.0f), color);

	glm::mat4 cabinetBack = cabinet;
	cabinetBack = glm::translate(cabinetBack, glm::vec3(-0.2f, 12.0f, 22.0f));
	drawCubeWood(cabinetBack, std::tuple<float, float, float>(21.1f, 36.0f, 1.0f), color);
	...
	for (float i = 0; i <= 1; i++) {
		...
		drawCubeWood(cabinetBottom, std::tuple<float, float, float>(21.0f, 21.0f, 1.0f), color);
	}
...
void drawBed(glm::mat4 room, std::tuple<float, float, float> color) {
	...
	drawCubeWood(frame, std::tuple<float, float, float>(bedSize.x, 5.0f, 1.0f), color);

	for (float i = 0; i < 2; i++) {
		glm::mat4 backFrame = bed;
		backFrame = glm::translate(backFrame, glm::vec3(0.0f, i*20.0f, -38.0f));
		drawCubeWood(backFrame, std::tuple<float, float, float>(bedSize.x, 5.0f, 1.0f), color);
	}
	for (float i = -1; i <= 1; i+=2) {
		for (float j = 0; j < 2; j++) {
			glm::mat4 frame = bed;
			...
			drawCubeWood(frame, std::tuple<float, float, float>(33.0f, 5.0f, 1.0f), color);
		}
	}
	...
	for (float i = -1; i <= 1; i+=2) {
		for (float j = -1; j <= 1; j+=2) {
			...
			drawCubeWood(leg, std::tuple<float, float, float>(5.0f, 30.0f, 5.0f), color);
		}
	}
}
